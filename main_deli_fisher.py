...
#            noisev = Variable(noise, volatile = True) # totally freeze netG
            fake = Variable(netG(noise.cuda()).data)
            inputv = fake
...
#        noisev = Variable(noise)
        ...
        vphi_fake = netD(fake)
        obj_G = -(vphi_fake.mean() + lambdaG*((2 - sigma.cuda())**2).mean()) # Just minimize mean difference
        obj_G.backward() # G: min_theta
        optimizerG.step()
        gen_iterations += 1

        mu.data += opt.rho * mu.grad.data #changed
        sigma.data += opt.rho * sigma.grad.data #changed
            
        mu.grad.data.zero_() #changed
        sigma.grad.data.zero_() #changed
...
#            image_slicer.save_tiles(tiles, directory='/home/hoosierai/Documents/github/DeliFGAN/samples/sliced',prefix='fake_samples_{0}.png'.format(gen_iterations))
    ...
    torch.save(sigma.data, '{0}/sigma_epoch_{1}.pth'.format(opt.experiment, epoch))	
